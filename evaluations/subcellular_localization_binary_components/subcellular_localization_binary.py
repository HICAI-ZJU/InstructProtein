...
class SubcellularLocalizationBinaryDataset(Dataset):

    def __init__(self, data_dir, data_name) -> None:
        super().__init__()
        env = lmdb.open(f'{data_dir}/subcellular_localization_2_{data_name}.lmdb', readonly=True, lock=False, readahead=False, meminit=False)
        self.sequences = []
        self.targets = []
...
class SubcellularLocalizationBinaryDataModule():
    ...
    def dataloader(self, data_name):
        dataset = SubcellularLocalizationBinaryDataset(self.data_dir, data_name)
        if data_name == 'train':
